...
def drawPlots(dataSet, fa, fc, scale = True, scaleFrom = -1 * 1e-12, scaleTo = 9 * 1e-12):
    ...
    fig.suptitle(
        'Wykresy pudełkowe wartości odprowadzeń dla CE (fa = {} fc = {})'.format(fa, fc),
        fontsize=16)
    fig.tight_layout()
    ...
    axs[0, 1].boxplot(dataSet[2], showmeans=True)
    axs[0, 1].set_title('Controls CE2')
    axs[0, 1].set_xticklabels(included)
    if scale:
        axs[0, 1].set_ylim(scaleFrom, scaleTo)

    axs[1, 0].boxplot(dataSet[4], showmeans=True)
    axs[1, 0].set_title('Depression CE1')
    axs[1, 0].set_xticklabels(included)
    if scale:
        axs[1, 0].set_ylim(scaleFrom, scaleTo)

    axs[1, 1].boxplot(dataSet[6], showmeans=True)
    axs[1, 1].set_title('Depression CE2')
    axs[1, 1].set_xticklabels(included)
    if scale:
        axs[1, 1].set_ylim(scaleFrom, scaleTo)

    axs[2, 0].boxplot(dataSet[8], showmeans=True)
    axs[2, 0].set_title('Remission CE1')
    axs[2, 0].set_xticklabels(included)
    if scale:
        axs[2, 0].set_ylim(scaleFrom, scaleTo)

    axs[2, 1].boxplot(dataSet[10], showmeans=True)
    axs[2, 1].set_title('Remission CE2')
    ...
    # plt.show()
    if scale:
        plt.savefig('Plots/BoxPlots/Oczy_osobno/Skalowane/CE_BoxPlot_Fa' + str(fa) + '_Fc' + str(fc) + '_scaled.png', dpi=100)
    else:
        plt.savefig('Plots/BoxPlots/Oczy_osobno/CE_BoxPlot_Fa' + str(fa) + '_Fc' + str(fc) + '.png', dpi=100)

    fig1, axs1 = plt.subplots(3, 2, figsize=(16, 8))
    fig1.suptitle(
        'Wykresy pudełkowe wartości odprowadzeń dla OE (fa = {} fc = {})'.format(fa, fc),
        fontsize=16)
    fig1.tight_layout()

    axs1[0, 0].boxplot(dataSet[1], showmeans=True)
    axs1[0, 0].set_title('Controls OE1')
    axs1[0, 0].set_xticklabels(included)
    if scale:
        axs[0, 0].set_ylim(scaleFrom, scaleTo)

    axs1[0, 1].boxplot(dataSet[3], showmeans=True)
    axs1[0, 1].set_title('Controls OE2')
    axs1[0, 1].set_xticklabels(included)
    if scale:
        axs[1, 1].set_ylim(scaleFrom, scaleTo)

    axs1[1, 0].boxplot(dataSet[5], showmeans=True)
    axs1[1, 0].set_title('Depression OE1')
    axs1[1, 0].set_xticklabels(included)
    if scale:
        axs[1, 0].set_ylim(scaleFrom, scaleTo)

    axs1[1, 1].boxplot(dataSet[7], showmeans=True)
    axs1[1, 1].set_title('Depression OE2')
    axs1[1, 1].set_xticklabels(included)
    if scale:
        axs[1, 1].set_ylim(scaleFrom, scaleTo)

    axs1[2, 0].boxplot(dataSet[9], showmeans=True)
    axs1[2, 0].set_title('Remission OE1')
    axs1[2, 0].set_xticklabels(included)
    if scale:
        axs[2, 0].set_ylim(scaleFrom, scaleTo)

    axs1[2, 1].boxplot(dataSet[11], showmeans=True)
    axs1[2, 1].set_title('Remission OE2')
    axs1[2, 1].set_xticklabels(included)
    if scale:
        axs[2, 1].set_ylim(scaleFrom, scaleTo)

    if scale:
        plt.savefig('Plots/BoxPlots/Oczy_osobno/Skalowane/OE_BoxPlot_Fa' + str(fa) + '_Fc' + str(fc) + '_scaled.png', dpi=100)
    else:
        plt.savefig('Plots/BoxPlots/Oczy_osobno/OE_BoxPlot_Fa' + str(fa) + '_Fc' + str(fc) + '.png', dpi=100)
